...
from geocoder import get_toponym, get_toponym_lonlat, get_toponym_spn, get_address, get_post_index, lon_lat_distance
from search_maps import get_org_lon_lat, search_org, get_org_name, get_org_address
...
class Window(QMainWindow):
    ...
    def update_map(self):
        ...
        if self.org:
            ...
            # По документации API индекс в ответе отсутствует
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if not self.search_org(dot, self.full_address.toPlainText()):
                return
            self.dot = get_org_lon_lat(self.org)
            self.update_map()
    ...
    def search_org(self, lo_la, search_text):
        ...
        if self.org is None or lon_lat_distance(lo_la, get_org_lon_lat(self.org)) > 50:
            self.org = None
            return False
